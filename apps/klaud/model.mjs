...
const runSettingsValues = { bashApproval: ["always", "auto", "ask", "whitelist"], reasoningEffort: ["default", "off", "low", "medium", "high"] };
...
export function updateTranscript(messages, event) {
  ...
  } else if (["THINKING_START", "THINKING_CONTENT", "THINKING_END"].includes(event.type)) {
    if (!string(event.messageId, 300)) throw new Error("Invalid thinking identifier.");
    if (event.type === "THINKING_CONTENT" && typeof event.delta !== "string") throw new Error("Invalid thinking delta.");
    if (event.type === "THINKING_END" && event.content !== undefined && typeof event.content !== "string") throw new Error("Invalid thinking text.");
    messageId = event.messageId;
    transform = existing => ({ ...existing, id: messageId, role: "assistant", content: existing?.content ?? "", thinking: event.type === "THINKING_END" && typeof event.content === "string" ? event.content : (existing?.thinking ?? "") + (event.type === "THINKING_CONTENT" ? event.delta : "") });
...
export function presentTranscript(messages, view = "activity") {
  ...
  for (const item of messages) {
    if (item.role === "user" || item.role === "assistant" && (item.content.trim() || item.content === "" && !item.completion && !item.toolCalls?.length)) rows.push({ ...item, toolCalls: undefined, ...(item.completion ? { completion: publicCompletion(item.completion) } : {}) });
