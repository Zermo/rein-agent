...
| Android (aarch64) | App userland overlay in a Termux home; verified boot, system partitions, and other apps stay untouched | A persistent Termux shell with Node 18+, a reachable model endpoint, and a backup before OS-level changes |
...
`rein os prepare --target android --output <new-directory>` stages the same verified payload for an Android app user (for example Termux). Its installer, `install-android.mjs`, confirms the machine identifies as Android (platform `android`, aarch64, `ro.build.version.release`) and that it runs as the app user whose home is an app-private userland home under `/data/.../files/home`, then creates `~/.local/share/rein-os` and `~/.local/bin/rein` in that home, refusing to overwrite either. It never touches the verified-boot base, the system partitions, or any other app. `REIN_OS_ANDROID_HOME` and `REIN_OS_ANDROID_RELEASE` override those two facts so the kit can be exercised on a staging host.

Android reports as `android` to the harness; `rein os plan` detects it on the host and reports the `android-userland` adapter with userland, model-endpoint, and backup gates. The phone does not host a model server: point `REIN_BASE_URL` at a reachable LAN or cloud endpoint before `rein setup`. Image-level Android (own bootable media) is not what this kit claims.
