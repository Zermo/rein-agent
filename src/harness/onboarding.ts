...
import { runBudgetSetup } from "./budget-setup.ts";
...
export async function runOnboarding(options: SetupOptions & { edition?: InstallEdition } = {}, dependencies: OnboardingDependencies = {}): Promise<number> {
	...
	try {
		...
		let connected = false;
		while (!connectLater) {
			const code = await (dependencies.setup ?? runSetup)({ ...options, maxTurns: undefined, maxIterations: undefined, status: !!reuse }, {
