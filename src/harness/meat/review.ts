...
import type { Context, Message, Tool, Model, AssistantMessage } from "../../ai/types.ts";
...
export interface MeatConnection {
	model: Model; apiKey?: string; toolsMode: "native" | "text";
	forcedMode: ToolMode; temperature?: number;
}
export interface ReviewOptions extends RunnerOptions {
	diff?: string; refs?: string[]; staged?: boolean; workingTree?: boolean;
	/** An agent tool supplies the already-resolved connection of its parent run. */
	connection?: MeatConnection;
...
export async function reviewDiff(cwd: string, options: Pick<ReviewOptions, "refs" | "staged" | "workingTree" | "signal"> = {}): Promise<string> {
	...
	const args = options.staged ? ["diff", ...safe, "--cached", "--"] : options.workingTree ? ["diff", ...safe, "HEAD", "--"]
		: shas.length === 2 ? ["diff", ...safe, ...shas, "--"] : ["show", "--format=", "--diff-merges=first-parent", ...safe, shas[0] ?? "HEAD", "--"];
...
export async function runMeatReview(options: ReviewOptions): Promise<MeatResult> {
	...
	const runner = options.connection ?? await createRunner({ ...options, tools: [], autoContext: false, systemPrompt: "", activityId: undefined });
	const { model, apiKey } = runner;
	const config = options.connection ? undefined : loadConfig();
	let toolsMode = runner.toolsMode, requests = 0;
	const modelCalls = new Set<Promise<AssistantMessage>>();
	...
	try { return await runMeatEngine({ diff, cwd: options.cwd, maxTurns: Math.min(options.maxTurns ?? 8, 24),
		...
		request: async (kind, payload) => {
			...
			const response = model.baseUrl.startsWith("cli://") ? streamCli(model, context, { signal: controller.signal, maxTokens: model.maxTokens })
				: stream(model, context, { apiKey, signal: controller.signal, maxTokens: model.maxTokens, toolsMode, temperature });
			const completion = response.result(); modelCalls.add(completion);
			let result: AssistantMessage;
			try { result = await completion; } finally { modelCalls.delete(completion); }
		...
		// The worker may finish before a provider subprocess has stopped. Drain
		// its result so callers cannot exit before owned process-group cleanup.
