...
test("real runner patches live shell and resumes the same bot transcript after restart", { timeout: 15_000 }, async t => {
	...
	for (const name of ["bash", "web_search", "web_fetch", "skill", "klaud_get_shell", "klaud_patch_shell", "arc_cua"]) assert.ok(toolNames.includes(name), name);
	assert.match(f.requests[0].messages[0].content, /^You are Fixture bot \(klaud-bot-[0-9a-f]{8}\)\. Klaud field unit: computer=local engine=openai-compat cwd=/);
