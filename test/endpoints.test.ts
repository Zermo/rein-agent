...
test("scheme-less private, NetBird, internal, and local ports normalize without losing explicit schemes", () => {
	for (const host of ["100.64.10.2:1234", "100.127.255.254:8000", "10.1.2.30:8123", "192.168.1.20:1234", "172.31.1.2:8080", "model-host.netbird.cloud:8123", "model-host", "localhost:11434", "[::1]:1234"]) {
		assert.equal(normalizeBaseUrl(host), `http://${host}/v1`);
	}
	assert.equal(normalizeBaseUrl("https://model-host.netbird.cloud:1234"), "https://model-host.netbird.cloud:1234/v1");
...
test("discovery distinguishes refused, DNS, timeout, bad path, and invalid model list", async (t) => {
	...
	assert.match((await detectEndpoint("100.64.0.1:8123")).error!, /Connection refused/);
	fake.mock.mockImplementation(async () => { throw new Error("fetch failed", { cause: { code: "ENOTFOUND" } }); });
	assert.match((await detectEndpoint("model-host.netbird.cloud:8123")).error!, /Host name could not be resolved/);
...
test("same-origin model-list redirects preserve the discovered API prefix", async (t) => {
	...
	const result = await detectEndpoint("100.64.1.2:8123");
	assert.equal(result.baseUrl, "http://100.64.1.2:8123/routed/v1");
...
test("saved API keys are scoped to exact normalized endpoint and SSH host", async () => isolated(async directory => {
	writeFileSync(join(directory, "config.json"), JSON.stringify({ provider: "custom", baseUrl: "http://127.0.0.1:8123/v1", apiKey: "saved-secret", sshHost: "model-host" }));
	assert.equal(apiKeyFor("custom", "127.0.0.1:8123/v1", "model-host"), "saved-secret");
	assert.equal(apiKeyFor("custom", "127.0.0.1:8123/v1", "other"), undefined);
	assert.equal(apiKeyFor("custom", "127.0.0.1:8123/v1"), undefined);
	assert.equal(apiKeyFor("custom", "127.0.0.1:8123/proxy/v1", "model-host"), undefined);
	assert.equal(apiKeyFor("custom", "100.64.2.1:8123/v1", "model-host"), undefined);
}));
...
test("provider environment keys only go to preset origins; generic REIN_API_KEY is explicit", async () => isolated(async () => {
	...
	assert.equal(apiKeyFor("openai", "http://100.64.0.1:8123/v1"), undefined);
...
test("changing endpoint ignores saved model/key and discovers only the new endpoint", async (t) => isolated(async directory => {
	...
	const model = await resolveModel({ baseUrl: "100.64.0.1:8123" });
	assert.equal(model.id, "new-model");
	assert.equal(model.baseUrl, "http://100.64.0.1:8123/v1");
...
test("saved normalized endpoint retains model and explicit SSH target drops stale model", async (t) => isolated(async directory => {
	writeFileSync(join(directory, "config.json"), JSON.stringify({ provider: "custom", baseUrl: "http://127.0.0.1:8123/v1", model: "saved-model", sshHost: "model-host" }));
	const model = await resolveModel({ baseUrl: "127.0.0.1:8123/v1" });
	assert.equal(model.id, "saved-model");
	assert.equal(model.sshHost, "model-host");
	const overridden = await resolveModel({ baseUrl: "127.0.0.1:8123/v1", sshHost: "other", model: "explicit" });
...
test("CLI routing avoids HTTP discovery, uses subscription default, and rejects mixed endpoint settings", async (t) => isolated(async directory => {
	...
	await assert.rejects(resolveModel({ provider: "copilot", sshHost: "model-host" }), /SSH forwarding/);
	...
	assert.equal((await resolveModel({ baseUrl: "100.64.1.2:8123", model: "local-model" })).provider, "custom");
}));
...
test("discovered root APIs retain their prefix through normalization and saved model resolution", async (t) => isolated(async directory => {
	...
	t.mock.method(globalThis, "fetch", async (url: string) => {
		requests.push(url);
		return url === "http://host:8123/models" ? Response.json({ data: [{ id: "root-model" }] }) : Response.json({}, { status: 404 });
	});
	const detected = await detectEndpoint("host:8123");
	assert.equal(detected.baseUrl, "http://host:8123/");
	...
	assert.equal(model.baseUrl, "http://host:8123/");
	assert.equal(apiKeyFor(model.provider, model.baseUrl), "root-key");
	assert.equal(apiKeyFor(model.provider, "http://host:8123/v1"), undefined);
}));


test("empty environment overrides do not erase a saved endpoint and model", async () => isolated(async directory => {
	writeFileSync(join(directory, "config.json"), JSON.stringify({ provider: "custom", baseUrl: "http://100.64.0.1:8123/v1", model: "saved-model" }));
